...
int shortestPath(const IGraph &graph, int vertex, int end) {
    ...
    while (!que.empty()) {
        ...
        std::vector<std::pair<int,int>> nextVert = graph.GetNextVertices(currentVertex);
        for (const auto &nextVertex: nextVert) {
            if (!visited[nextVertex.first]) {
                que.emplace(distance[nextVertex.first], nextVertex.first);
                distance[nextVertex.first] = distance[currentVertex] + nextVertex.second;
                visited[nextVertex.first] = true;
            } else if (distance[nextVertex.first] > distance[currentVertex] + nextVertex.second) {
                que.erase(std::make_pair(distance[nextVertex.first], nextVertex.first));
                distance[nextVertex.first] = distance[currentVertex] + nextVertex.second;
                que.emplace(distance[nextVertex.first], nextVertex.first);
            }
        }
...
int main() {
    ...
    int start, end;
    std::cin >> start >> end;
    std::cout << shortestPath(graph, start, end) << std::endl;
    return 0;
}